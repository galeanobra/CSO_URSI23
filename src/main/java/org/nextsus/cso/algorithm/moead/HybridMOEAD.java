package org.nextsus.cso.algorithm.moead;
...
public class HybridMOEAD<S extends Solution<?>> extends AbstractMOEAD<S> {
    public HybridMOEAD(Problem<S> problem, int populationSize, int resultPopulationSize, int maxEvaluations, MutationOperator<S> mutation, CrossoverOperator<S> crossover, FunctionType functionType, String dataDirectory, double neighborhoodSelectionProbability, int maximumNumberOfReplacedSolutions, int neighborSize) {
    ...
    public void run() {
        ...
        do {
            ...
            for (int i = 0; i < populationSize; i++) {
                ...
                ((StaticCSO) problem).intelligentSwitchOff((BinaryCSOSolution) child);
