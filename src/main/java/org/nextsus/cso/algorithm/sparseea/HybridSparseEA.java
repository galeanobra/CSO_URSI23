...
//        tabuSize = 5;
    ...
    public void run() {
        ...
        CrowdingDistanceDensityEstimator<S> distance = new CrowdingDistanceDensityEstimator<>();
...
//        tabuSize = (Integer) getInputParameter("tabuSize");
        ...
        List<S> q = generatePopulation(d, d, mask);  // dec = dec x mask because mask = identity matrix

        // Return non-dominated fronts
        Ranking<S> ranking = new FastNonDominatedSortRanking<>(this.dominanceComparator).compute(q);
        score = new int[d];
        ...
        while (this.evaluations < this.maxEvaluations) {
            ...
            set.addAll(variation(_population, score, mask));
            ...
            while ((remain > 0) && (remain >= front.size())) {
                ...
                //Obtain the next front
                index++;
                if (remain > 0) front = ranking.getSubFront(index);
            }
        ...
        this.population = ranking.getSubFront(0);
    }
     ...
     */
    private List<S> generatePopulation(int populationSize, int d, boolean[][] variables) {
...
//            problem.evaluateConstraints(s);
     ...
     */
    public List<S> variation(List<BinaryCSOSolution> _population, int[] score, boolean[][] mask) {
        ...
        List<S> _population_variation = new ArrayList<>(_population.size());
        for (boolean[] b : oMask) {
            BinaryCSOSolution s = new BinaryCSOSolution(Collections.singletonList(this.numberOfBits), problem.numberOfObjectives());
            BitSet bits = s.variables().get(0);
